...
/**
 * Claude Code's official deep-link scheme (`claude-cli://`). Per Anthropic's deep-links doc
 * (code.claude.com/docs/en/deep-links.md, verified 2026-09-21), Claude Code registers a
 * `claude-cli://` URL handler with the OS (on macOS a `~/Applications/Claude Code URL
 * Handler.app`, installed the first time you send a prompt in an interactive session). The
 * ONLY path the handler accepts is `claude-cli://open`; it takes `cwd` (an absolute working
 * directory), `q` (the prompt prefill), and `repo` (an owner/name slug). We use `cwd` + `q`.
 *
 * History / why this changed (work-098): the launcher previously emitted
 * `claude://code/new?q=…&folder=…`. That path+param shape is **not documented and not
 * honored** — `code/new` is not a real handler path and `folder` is not a real param, so a
 * click landed the Owner in Claude Code's "No folder." state. `open`'s exit 0 on the old URL
 * was a false positive: it only proved *some* app claims the bare `claude:` scheme, not that
 * the path/params were understood.
 */
export const CLAUDE_CLI_SCHEME = "claude-cli:";

/**
 * The deep link that opens a NEW Claude Code session in `cwd` with `prompt` prefilled
 * (ADR-016 / work-046, corrected in work-098). Emits the documented
 * `claude-cli://open?cwd=<abs>&q=<encoded>` form: `cwd` is the absolute working directory
 * (the handler rejects `..` segments and network/UNC paths, so callers pass a resolved real
 * path) and `q` is the percent-encoded prompt — placed in the composer and *sent only when
 * the human presses Enter*, never auto-sent (ADR-016). The `[scope-creep-thread:<id>]` marker
 * inside `prompt` survives into `q` for correlation (work-047). NOTE: the `claude-cli:` handler
 * must be OS-registered; the server verifies it (see claude-sessions.server
 * `verifyClaudeCliScheme`) and the UI degrades honestly to the copyable command when it isn't.
 */
 ...
 * (the open floor). Same documented `claude-cli://open` handler, `cwd` only. This opens a NEW
 * session — it is never a "resume" of an existing one (resume-by-uuid is CLI-only; see
 * buildResumeCommand).
 */
